...
class CaseHistory:
    ...
    def display_case_history():
        ...
        with CaseManager() as case_manager:
            ...
            if search_query:
                cases = case_manager.search_cases(user_id, search_query)
            else:
                status = None if status_filter == 'All' else status_filter
                cases = case_manager.get_user_cases(user_id, status=status)
    ...
    def _display_case_card(case: Case, case_manager: CaseManager):
        ...
        with st.container():
            ...
            with col2:
                st.markdown(f"<span style='color: {status_color}; font-weight: bold;'>●</span> **{case.status.upper()}**", 
            ...
            with col4:
                ...
                if case.days_remaining is not None:
                    ...
                    st.markdown(f"<span style='color: {days_color}; font-weight: bold;'>Days Left: {case.days_remaining}</span>", 
                               unsafe_allow_html=True)
