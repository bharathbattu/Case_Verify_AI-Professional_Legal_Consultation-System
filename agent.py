...
# Load data files
with open('rules/limitation.json') as f:
    LIMIT = json.load(f)

with open('rules/forum.json') as f:
    FORUM = json.load(f)

with open('rules/court_hierarchy.json') as f:
    COURT_HIERARCHY = json.load(f)

with open('rules/detailed_provisions.json') as f:
    DETAILED_PROVISIONS = json.load(f)

with open('rules/language_support.json', encoding='utf-8') as f:
...
def get_cache_key(facts: str, relief: str, pin: str, case_type: str = None) -> str:
...
def analyse(facts: str, relief: str, pin: str, case_type: str = None) -> Dict[str, Any]:
    """
