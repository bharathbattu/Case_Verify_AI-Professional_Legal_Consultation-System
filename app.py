...
# O-01: Configure structured JSON logging (replaces ad-hoc basicConfig)
configure_logging()
logger = logging.getLogger(__name__)

# Op-04: Register graceful shutdown hooks (atexit + signal handlers)
...
def format_analysis_result(result):
    ...
    # Get comprehensive analysis data — S-07: escape all dynamic strings for XSS safety
    verdict = esc(result.get('verdict', 'Analysis Complete'))
    days_left = result.get('days_left', 0)
    limitation = esc(result.get('limitation', 'Not Specified'))
    deadline = esc(result.get('deadline', 'Not Determined'))
    confidence = result.get('confidence_score', 0)
    reasoning = esc(result.get('legal_reasoning', 'Legal analysis based on provided facts'))
    ...
    case_strength = esc(result.get('case_strength', 'Assessment depends on evidence quality and legal merits'))
    timeline = esc(result.get('estimated_timeline', '6-24 months typically for court proceedings'))
    costs = esc(result.get('likely_costs', 'Legal costs vary based on case complexity'))
    ...
    if isinstance(practical_advice, list):
        practical_advice_formatted = "  \n".join([f"• {esc(advice.lstrip('0123456789. '))}" for advice in practical_advice])
    else:
        practical_advice_formatted = esc(practical_advice)
    
    # Format strategic recommendations
    if isinstance(strategic_recommendations, list):
        strategic_recommendations_formatted = "  \n".join([f"• {esc(rec.lstrip('0123456789. '))}" for rec in strategic_recommendations])
    else:
        strategic_recommendations_formatted = esc(strategic_recommendations)
        
    # Format precedent references
    if isinstance(precedent_references, list):
        precedent_references_formatted = "  \n".join([f"• {esc(ref.lstrip('0123456789. '))}" for ref in precedent_references])
    else:
        precedent_references_formatted = esc(precedent_references)
        
    # Format risk factors
    if isinstance(risk_factors, list):
        risk_factors_formatted = "  \n".join([f"• {esc(risk.lstrip('0123456789. '))}" for risk in risk_factors])
    else:
        risk_factors_formatted = esc(risk_factors)
    ...
    court_name = esc(court_info.get('name', 'Appropriate Jurisdiction Court'))
    court_jurisdiction = esc(court_info.get('jurisdiction', 'As per territorial jurisdiction'))
...
<strong>Case Category:</strong> {esc(result.get('ai_analysis', {}).get('cause_identified', 'Civil/Commercial Matter'))}  
<br><br>
<strong>Procedural Notes:</strong> {esc(result.get('jurisdiction_notes', 'File in appropriate court based on territorial and pecuniary jurisdiction'))}
</div>
...
**Assessment Date:** {esc(result.get('ai_analysis', {}).get('date_reasoning', 'Based on comprehensive factual and legal analysis'))}  
...
def render_case_analysis_interface():
    ...
    st.markdown(f'<div class="case-number">📋 Case No: {esc(case_number)}</div>', unsafe_allow_html=True)
        ...
        if not facts.strip():
            ...
            try:
                ...
                <div style="background: linear-gradient(135deg, #f39c12, #e67e22); color: white; padding: 1.5rem; border-radius: 10px; border: 2px solid #e67e22;">
                    ...
                    <p style="margin: 0.5rem 0 0 0;">{esc(error_message)}</p>
                ...
                <div style="background: linear-gradient(135deg, #e74c3c, #c0392b); color: white; padding: 1.5rem; border-radius: 10px; border: 2px solid #c0392b;">
                    ...
                    <details style="margin-top: 1rem;">
                        ...
                        <p style="font-family: monospace; font-size: 0.8rem; margin: 0.5rem 0 0 0;">
                            <strong>Type:</strong> {esc(error_type)}<br>
                            <strong>Message:</strong> {esc(error_details)}
                        </p>
                    </details>
                </div>
                """, unsafe_allow_html=True)
